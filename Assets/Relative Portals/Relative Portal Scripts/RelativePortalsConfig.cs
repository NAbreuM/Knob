...
public class RelativePortalsConfig : MonoBehaviour {
    ...
    //Adding a field for the player transform. tpObject will now mirror this instead of the camera.
    ...
    void Start() {
        //Find the Avatar Transform if possible
        if(CamCorrectGameObjects.Count > 0) {
            AvatarTransform = CamCorrectGameObjects[0].transform;
        }
    ...
    void LateUpdate() {
        if(!ERROR) {
            ...
            if(ColliderContainsPoint(Col_PortalA.transform, Cam_Player.transform.position, true) && !inPortalA && AllowTeleport) {
                ...
                //I've changed the script so that tpObject reflects avatar transform instead of camera transform.
                tpObject.transform.rotation = GO_PortalB.transform.rotation * Quaternion.Inverse(GO_PortalA.transform.rotation) * AvatarTransform.rotation;
                Vector3 posB = GO_PortalA.transform.InverseTransformPoint(AvatarTransform.position);
                ...
                tpObject.transform.rotation = GO_PortalA.transform.rotation * Quaternion.Inverse(GO_PortalB.transform.rotation) * AvatarTransform.rotation;
                Vector3 posA = GO_PortalB.transform.InverseTransformPoint(AvatarTransform.position);
