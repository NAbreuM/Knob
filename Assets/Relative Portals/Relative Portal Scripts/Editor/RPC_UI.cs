...
[CustomEditor(typeof(RelativePortalsConfig))]
public class RPC_UI : Editor {
    RelativePortalsConfig SPC;
    ...
    void OnEnable() {
        SPC = (RelativePortalsConfig)target;
        DefaultGUIColour = GUI.color;
        if(SPC.editorImage != null)
            titleImage = SPC.editorImage;
        ERROR.wordWrap = true;
